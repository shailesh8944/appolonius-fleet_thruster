...
class SimpleEKFNode(Node):
    def __init__(self):
        ...
        self.uwb_sub = self.create_subscription(
            ...
            self.uwb_callback,
            10)
        self.psi_pub =self.create_publisher(Float32, '/kf/yaw', 10)  
    ...
    def publish_odom(self):
        ...
        odom_msg.twist.twist.linear.x=float(self.x[0,0])
        odom_msg.twist.twist.linear.y=float(self.x[1,0])
        odom_msg.twist.twist.angular.z = float(self.x[2, 0])
        odom_msg.pose.covariance[0] = psi
        self.odom_pub.publish(odom_msg)

        # Publish KF yaw separately
        psi_msg = Float32()
        psi_msg.data = psi
        self.psi_pub.publish(psi_msg)

        self.get_logger().info(f"[ODOM] Published KF ψ = {psi:.2f} rad = {np.degrees(psi):.2f}°")
        self.odom_pub.publish(odom_msg)
