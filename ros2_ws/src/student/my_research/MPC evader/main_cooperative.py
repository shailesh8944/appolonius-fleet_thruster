...
class PursuerPublisher(Node):
    ...
    def __init__(self):
        ...
        self.evader_rudder_pub = self.create_publisher(Float32, '/evader/rudder_command', 10)
    ...
    def publish_evader_rudder(self, rudder):
        msg = Float32()
        msg.data = float(rudder)
        self.evader_rudder_pub.publish(msg)
        self.get_logger().info(f"Published evader rudder: {math.degrees(rudder):.2f} deg")
...
def run_simulation_with_live_vis():
    ...
    d = 8# Distance from evader to each pursuer (adjust as needed)
    center_x, center_y = 23,6

    # Evader at center
    X0_evader = np.array([0.8, .001, .01, center_x, center_y, .01, .01, 8])

    # Equilateral triangle around evader
    X0_pursuer1 = np.array([0.2, .001, .01, center_x + d, center_y, .06, .001, 4])
    X0_pursuer2 = np.array([0.2, .001, .01, center_x - d/2, center_y + d * np.sqrt(3)/2, .02, .001, 4])
    X0_pursuer3 = np.array([0.2, .001, .01, center_x - d/2, center_y - d * np.sqrt(3)/2, 0.1, .001, 4])
...
# ...existing code...
    ...
    for t in tqdm(time[:-1], desc="Simulation Progress"):
        ...
        Xe[0] = 2.8 # Update surge speed for evader
        ...
        if not node.evader_state_updated:
            ...
            continue
        node.publish_state(Xp1, Xp2, Xp3)
        ...
        nmpc_e,cost_e=evader_controller.nlpsolve_with_cost(Xe, Xp1, Xp2, Xp3, goal_evader, time, obs_pos, return_cost=True)
        print(f"Evader - Cost: {cost_e:.2f}")
        pursuer_states = [Xp1, Xp2, Xp3]
        V_list = [0.5]*8  # alpha_dot constatnt for all pursuers
         # Update surge speed for evader
        ...
        node.evader_state[6] = control_e[0,1]
        ...
        control_p1 = np.clip(control_p1, [0.2, -0.61], [4, 0.61])
        control_p2 = np.clip(control_p2, [0.2, -0.61], [4,0.61])
        control_p3 = np.clip(control_p3, [0.2, -0.61], [4, 0.61])
        control_e = np.clip(control_e, [5, -0.61], [6, 0.61])
        ...
        node.publish_evader_rudder(control_e[0, 1])
        ...
        Xe[0] = 1.2  # Update surge speed for evader
