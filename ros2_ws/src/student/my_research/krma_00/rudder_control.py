...
def generate_evasion_rudder(current_state, pursuer_states, goal_state):
    ...
    """
    # --- 1. Adjusted gains for better evasion ----------------
    K_att = 0.005        # Reduced attractive gain to prioritize evasion
    K_rep_base = 400000.0  # Increased base repulsive gain
    d0 = 800.0          # Increased influence radius for earlier evasion
    eps = 1e-6
    max_tc = 30.0       # Reduced max time-to-collision for quicker response

    # Increased PD gains for more aggressive maneuvers
    Kp = 1.5            # Increased proportional gain
    Kd = 0.8            # Increased derivative gain
    max_rudder = np.radians(35)

    # --- 2. Calculated weighted attractive force -------------
    pos = np.array(current_state[3:5])
    goal = np.array(goal_state)
    dist_to_goal = np.linalg.norm(goal - pos)
    
    # Reduce attraction when pursuers are close
    closest_pursuer_dist = min(np.linalg.norm(np.array(p[3:5]) - pos) for p in pursuer_states)
    attraction_weight = np.clip(closest_pursuer_dist / (2*d0), 0.1, 1.0)
    F_att = K_att * attraction_weight * (goal - pos)

    # --- 3. Enhanced repulsive forces -----------------------
    F_rep = np.zeros(2)
    for p in pursuer_states:
        p_pos = np.array(p[3:5])
        p_vel = np.array(p[5:7])
        ...
        # Enhanced closing speed calculation
        rel_vel = p_vel
        closing_speed = max(0.1, np.dot(rel_vel, -diff/dist))
        t_c = dist/closing_speed
        t_c = min(t_c, max_tc)

        # Exponential scaling of repulsion based on distance
        K_rep = K_rep_base * np.exp(-dist/d0) / (1.0 + 0.5*t_c)

        if dist < d0:
            # Modified repulsion formula for stronger close-range response
            rep_mag = K_rep * (1.0/dist - 1.0/d0) / (dist**1.5)  # Changed power from 2 to 1.5
            F_rep += rep_mag * (diff / dist)

    # --- 4. Modified force combination ---------------------
    F_total = F_att +  F_rep  # Increased weight on repulsion

    # --- 5. Enhanced heading control ----------------------
    if np.linalg.norm(F_total) < eps:
        # If no clear guidance, turn away from closest pursuer
        closest_pursuer = min(pursuer_states, key=lambda p: np.linalg.norm(np.array(p[3:5]) - pos))
        diff_closest = pos - np.array(closest_pursuer[3:5])
    ...
    # Add extra rudder command when pursuers are very close
    emergency_turn = 0.0
    if closest_pursuer_dist < d0/2:
        emergency_turn = np.sign(err) * 0.5 * max_rudder
