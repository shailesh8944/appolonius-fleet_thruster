...
class VesselROSBridge(Node):
    ...
    durability=DurabilityPolicy.VOLATILE
        ...
        self.pub_evader_goal = self.create_publisher(Point, '/evader_03/goal', 10)
        ...
        self.state_p3 = None
        self.state_e  = None
        # Last published actuator commands, used to augment odom(12) -> nmpc state(14)
        self.last_act = {
    ...
    def _odom_to_state(self, msg: Odometry, vessel_key: str):
        ...
        eul = quat_to_eul(quat, order='ZYX')  # [roll, pitch, yaw]
        state_12 = np.array([
        ...
        # Only evader NMPC needs actuator states in X=[... ,act_port, act_stbd].
        if vessel_key != 'e':
            return state_12

        with self._lock:
            act_est = self.last_act['e'].copy()
        return np.concatenate([state_12, act_est]).astype(float)

    def p1_callback(self, msg: Odometry):
        st = self._odom_to_state(msg, 'p1')
        st[0] = 0.5  # force P1 surge
        with self._lock:
            self.state_p1 = st

    def p2_callback(self, msg: Odometry):
        st = self._odom_to_state(msg, 'p2')
        with self._lock:
            self.state_p2 = st

    def p3_callback(self, msg: Odometry):
        st = self._odom_to_state(msg, 'p3')
        with self._lock:
            self.state_p3 = st

    def e_callback(self, msg: Odometry):
        st = self._odom_to_state(msg, 'e')
        with self._lock:
            self.state_e = st
    ...
    def publish_thrust_p1(self, thrusts):
        ...
        T_port, T_stbd = float(thrusts[0]), float(thrusts[1])
        with self._lock:
            self.last_act['p1'][:] = [T_port, T_stbd]
        actuator_msg.actuator_values = [T_stbd, T_port]
        actuator_msg.actuator_names = ["th_stbd", "th_port"]
        actuator_msg.covariance = [0.01, 0.01]
    ...
    def publish_thrust_p2(self, thrusts):
        ...
        T_port, T_stbd = float(thrusts[0]), float(thrusts[1])
        with self._lock:
            self.last_act['p2'][:] = [T_port, T_stbd]
        actuator_msg.actuator_values = [T_stbd, T_port]
        actuator_msg.actuator_names = ["th_stbd", "th_port"]
        actuator_msg.covariance = [0.01, 0.01]
    ...
    def publish_thrust_p3(self, thrusts):
        ...
        T_port, T_stbd = float(thrusts[0]), float(thrusts[1])
        with self._lock:
            self.last_act['p3'][:] = [T_port, T_stbd]
        actuator_msg.actuator_values = [T_stbd, T_port]
        actuator_msg.actuator_names = ["th_stbd", "th_port"]
        actuator_msg.covariance = [0.01, 0.01]
        self.pub_p3.publish(actuator_msg)

    def publish_evader_goal(self, goal_xy):
        msg = Point()
        msg.x = float(goal_xy[0])
        msg.y = float(goal_xy[1])
        msg.z = 0.0
        self.pub_evader_goal.publish(msg)
...
# ===================== SIMULATION FUNCTION =====================ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
def run_simulation_with_live_vis(ros_bridge: VesselROSBridge):
    ...
    commanded_rudders_p1 = commanded_rudders_p2 = commanded_rudders_p3 = commanded_rudders_e = None
    commanded_props_p1, commanded_props_p2, commanded_props_p3, commanded_props_e = [], [], [], []
    commanded_port_act_p1, commanded_stbd_act_p1 = [], []
    commanded_port_act_p2, commanded_stbd_act_p2 = [], []
    commanded_port_act_p3, commanded_stbd_act_p3 = [], []
    commanded_port_act_e, commanded_stbd_act_e = [], []
    evader_act_prev = None
    evader_act_rate = 10.0  # max change per second in actuator command (relaxed)
    pursuer_act_prev = [None, None, None]
    pursuer_act_rate = 0.3  # max change per second in actuator command
    evader_diag_history = []
    all_predicted_paths = []
    ...
    for t in tqdm(time_vec[:-1], desc="Simulation Progress"):
        ...
        dt_ctrl = loop_start_wall - last_loop_wall
        # First iteration fallback and numerical guard.
        dt_ctrl = float(dt_ctrl if dt_ctrl > 1e-4 else time_step)
        last_loop_wall = loop_start_wall
        # Stable dt for pursuer low-level heading filter/rate logic.
        dt_pursuer_ctrl = float(np.clip(dt_ctrl, 0.05, 0.20))
        ...
        T_port_p1, T_stbd_p1 = velocity_to_controls1(
            cooperative_commands[0], Xp1, Xe, dt=dt_pursuer_ctrl, vessel_id="p1"
        )
        control_p1 = np.array([[T_port_p1, T_stbd_p1]], dtype=float)

        
        T_port_p2, T_stbd_p2 = velocity_to_controls1(
            cooperative_commands[1], Xp2, Xe, dt=dt_pursuer_ctrl, vessel_id="p2"
        )
        control_p2 = np.array([[T_port_p2, T_stbd_p2]], dtype=float)

        T_port_p3, T_stbd_p3 = velocity_to_controls1(
            cooperative_commands[2], Xp3, Xe, dt=dt_pursuer_ctrl, vessel_id="p3"
        )
        ...
        # Keep NMPC prediction step consistent with actual wall-clock controller update period.
        evader_controller.time_step = dt_ctrl
        t_nmpc = np.array([0.0, dt_ctrl], dtype=float)
        solve_t0 = time.perf_counter()
        nmpc_e, predicted_path = evader_controller.nlpsolve_with_cost(
            Xe, Xp1, Xp2, Xp3, goal_evader, t_nmpc, obs_pos,
            psi_reference=float(evader_debug['psi_selected'])
        )
        solve_dt = time.perf_counter() - solve_t0
        # print(f"[MPC] dt_ctrl={dt_ctrl:.3f}s, solve_time={solve_dt:.3f}s")
        all_predicted_paths.append(predicted_path)

        control_e = np.clip(nmpc_e, -1.0, 1.0)
        act_port_e, act_stbd_e = float(control_e[0]), float(control_e[1])
        if evader_act_prev is None:
            evader_act_prev = np.array([act_port_e, act_stbd_e], dtype=float)
        max_step = max(evader_act_rate * dt_ctrl, 1e-6)
        act = np.array([act_port_e, act_stbd_e], dtype=float)
        delta = np.clip(act - evader_act_prev, -max_step, max_step)
        act = evader_act_prev + delta
        evader_act_prev = act
        act_port_e, act_stbd_e = float(act[0]), float(act[1])
        ...
        # Rate-limit pursuer thruster commands
        max_step = max(pursuer_act_rate * dt_ctrl, 1e-6)
        for i, (t_port, t_stbd) in enumerate(
            [(T_port_p1, T_stbd_p1), (T_port_p2, T_stbd_p2), (T_port_p3, T_stbd_p3)]
        ):
            if pursuer_act_prev[i] is None:
                pursuer_act_prev[i] = np.array([t_port, t_stbd], dtype=float)
            act = np.array([t_port, t_stbd], dtype=float)
            delta = np.clip(act - pursuer_act_prev[i], -max_step, max_step)
            act = pursuer_act_prev[i] + delta
            pursuer_act_prev[i] = act
            if i == 0:
                T_port_p1, T_stbd_p1 = float(act[0]), float(act[1])
            elif i == 1:
                T_port_p2, T_stbd_p2 = float(act[0]), float(act[1])
            else:
                T_port_p3, T_stbd_p3 = float(act[0]), float(act[1])
        
       

        commanded_port_act_p1.append(float(T_port_p1))
        commanded_stbd_act_p1.append(float(T_stbd_p1))
        commanded_port_act_p2.append(float(T_port_p2))
        commanded_stbd_act_p2.append(float(T_stbd_p2))
        commanded_port_act_p3.append(float(T_port_p3))
        commanded_stbd_act_p3.append(float(T_stbd_p3))
        commanded_port_act_e.append(float(act_port_e))
        commanded_stbd_act_e.append(float(act_stbd_e))
        ...
        try:
            ...
            ros_bridge.publish_evader_goal(goal_evader)
...
if __name__ == "__main__":
    ...
    plots_dir = "thruster_simulation_05"
    ...
    (states_pursuer1, states_pursuer2, states_pursuer3, states_evader,
     ...
     all_pred_path, evader_diag_history,
     cmd_port_act_p1, cmd_stbd_act_p1,
     cmd_port_act_p2, cmd_stbd_act_p2,
     cmd_port_act_p3, cmd_stbd_act_p3,
     cmd_port_act_e, cmd_stbd_act_e) = simulation_results
    ...
    generate_analysis_plots(states_pursuer1, states_pursuer2, states_pursuer3,
                            states_evader, None, None, None, None,
                            cmd_prop_p1, cmd_prop_p2, cmd_prop_p3, cmd_prop_e,
                            plots_dir=plots_dir,
                            port_act_p1=cmd_port_act_p1, stbd_act_p1=cmd_stbd_act_p1,
                            port_act_p2=cmd_port_act_p2, stbd_act_p2=cmd_stbd_act_p2,
                            port_act_p3=cmd_port_act_p3, stbd_act_p3=cmd_stbd_act_p3,
                            port_act_e=cmd_port_act_e, stbd_act_e=cmd_stbd_act_e,
                            time_step=time_step)
