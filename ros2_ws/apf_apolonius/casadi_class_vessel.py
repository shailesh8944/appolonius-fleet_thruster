...
class Vessel:
    ...
    def __init__(self, vessel_params: Dict, hydrodynamic_data: Dict,ros_flag: bool = False):
        ...
        self.thrust_scale = float(vessel_params.get('thrust_scale', 10.0))
    ...
    def _dimensionalize_coefficients(self, rho, L, U):
        ...
        # Thruster coefficients are handled via thrust_scale.
    ...
    def thruster_forces(self, act_port, act_stbd):
        ...
        tau = ca.SX.zeros(6, 1)
        tau[0] = T_port + T_stbd
        # Match simulator r x F convention for port(y<0), stbd(y>0):
        # Nz = lever * (T_port - T_stbd)
        tau[5] = self.thruster_lever * (T_port - T_stbd)
        return tau
    ...
    def create_integrator(self, xdot, x, u, dt):
        """
        Fixed-step RK4 integrator to avoid CVODES failures inside the NLP.
        """
