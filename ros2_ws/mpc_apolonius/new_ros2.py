...
class VesselROSBridge(Node):
    ...
    def e_callback(self, msg):
        ...
        # Enforce fixed evader surge speed (m/s)
        try:
            self.state_e[0] = 0.55
        except Exception:
            pass
...
def run_simulation_with_live_vis(ros_bridge):
    ...
    for t in tqdm(time[:-1], desc="Simulation Progress"):
        ...
        # Enforce evader surge speed immediately after reading live state so all calculations use 0.55
        if ros_bridge.state_e is not None:
            try:
                ros_bridge.state_e[0] = 0.55
                Xe[0] = 0.55
            except Exception:
                pass
        ...
        # _, predicted_path = evader_controller.nlpsolve_with_cost(Xe, Xp1, Xp2, Xp3, goal_evader, time, obs_pos)
        # all_predicted_paths.append(predicted_path)
        # for i in range(NP):
        #     control_e = [nmpc_e[i, 0], nmpc_e[i, 1]]
        rudder_cmd=float(generate_evasion_rudder(Xe, [Xp1, Xp2, Xp3], goal_evader))
        control_e = np.array([[2, rudder_cmd]])
        ...
        print(f"Time {t:.1f}s: Cmd Rudders (deg) P1: {control_p1[0,1]:.2f}, rudder_P2: {control_p2[0,1]:.2f}, rudder_P3: {control_p3[0,1]:.2f}, rudder_E: {control_e[0,1]:.2f}")
