...
class VesselROSBridge(Node):
    ...
    durability=DurabilityPolicy.VOLATILE
        ...
        # Publishers
        self.pub_evader = self.create_publisher(Actuator, '/sookshma_02/actuator_cmd', 10)
        self.pub_p1     = self.create_publisher(Actuator, '/sookshma_01/actuator_cmd', 10)
        self.pub_p2     = self.create_publisher(Actuator, '/sookshma2_01/actuator_cmd', 10)
        self.pub_p3     = self.create_publisher(Actuator, '/sookshma_04/actuator_cmd', 10)

        # Subscribers (Odometry)
        self.sub_p1 = self.create_subscription(Odometry, '/sookshma_01/odometry_test', self.p1_callback, self.qos_odom)
        self.sub_p2 = self.create_subscription(Odometry, '/sookshma2_01/odometry_sim', self.p2_callback, self.qos_odom)
        self.sub_p3 = self.create_subscription(Odometry, '/sookshma_04/odometry_test', self.p3_callback, self.qos_odom)
        self.sub_e  = self.create_subscription(Odometry, '/sookshma_02/odometry_test', self.e_callback,  self.qos_odom)
...
def run_simulation_with_live_vis(ros_bridge: VesselROSBridge):
    ...
    for t in tqdm(time_vec[:-1], desc="Simulation Progress"):
        ...
        commanded_rudders_e.append(0.0)
