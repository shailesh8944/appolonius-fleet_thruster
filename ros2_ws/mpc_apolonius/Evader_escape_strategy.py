...
def compute_overlapping_angle_and_derivs(i, j, lam, centers, radii):
    ...
    dtheta_dri = numerical_partial_derivative(theta_from_r, [r_i, r_j], idx=0)  #  ∂θᵢⱼ/∂rᵢ → perturb rᵢ (index 0).
    dtheta_drj = numerical_partial_derivative(theta_from_r, [r_i, r_j], idx=1)  #   ∂θᵢⱼ/∂rⱼ → perturb rⱼ (index 1).
...
def compute_evader_heading_ramana(pursuer_states, evader_state, return_debug=False, debug_store=None):
    ...
    debug = {
        ...
        'phi_i': float(φ_i),
        'phi_j': float(φ_j),
        'r_i': float(radii[i_idx]),
        'r_j': float(radii[j_idx]),
