...
from cooperative_pursuer import (
    ...
    sort_pursuers_by_angle,check_capture
)
...
class VesselROSBridge(Node):
    ...
    def e_callback(self, msg):
        self.get_logger().info("Received odometry for evader")
        quat = np.array([
        msg.pose.pose.orientation.w,
        msg.pose.pose.orientation.x,
        msg.pose.pose.orientation.y,
        ...
        # Convert quaternion to Euler angles
        eul = quat_to_eul(quat, order='ZYX')
        self.state_e=np.array([msg.twist.twist.linear.x,
         msg.twist.twist.linear.y,
         msg.twist.twist.linear.z,
         msg.twist.twist.angular.x,
         msg.twist.twist.angular.y,
    ...
    def publish_rudder(self, pub, rudder_deg):
        msg = Actuator()
        # Publish rudder directly in degrees
...
def run_simulation_with_live_vis(ros_bridge):
    ...
    NP = 10
    NC = 4
    Q = np.array([100, 100, 4000])  # Weights: [goal_dist, goal_align, heading_track]
    time_step = 0.1
    simulation_time = 25
    ...
    for t in tqdm(time[:-1], desc="Simulation Progress"):
        ...
        print("Pursuer states (pos, heading_deg, yaw):")
        for i, Xp in enumerate([Xp1, Xp2, Xp3], start=1):
         print(f"P{i} pos={Xp[6:8]}, heading_deg={np.degrees(Xp[11]):.1f}, yaw={Xp[5]:.3f}")
        ...
        # Evader strategy diagnostics (Ramana & Kothari)
        _, evader_debug = compute_evader_heading_ramana(
            pursuer_states, Xe, return_debug=True
        )
        ...
        for i in range(NP):
            control_e[i] = nmpc_e[i]
            print(f"Evader NMPC step {i}: rudder_cmd={control_e[i]:.2f}")


        print("Generated evasion rudder command ___main file (deg):",control_e[0])
        control_p1 = np.clip(control_p1, [0, -35], [2, 35])
        control_p2 = np.clip(control_p2, [0, -35], [2, 35])
        control_p3 = np.clip(control_p3, [0, -35], [2, 35])
        control_e  = np.clip(control_e, -35, 35)
        ...
        print(f"Time {t:.1f}s: Cmd Rudders (deg) P1: {control_p1[0,1]:.2f}, rudder_P2: {control_p2[0,1]:.2f}, rudder_P3: {control_p3[0,1]:.2f}, rudder_E: {control_e[0]:.2f}")

        # --- Publish rudder commands (angles) ---
        try:
            ros_bridge.publish_rudder(ros_bridge.pub_evader, control_e[0])
        ...
        if ros_bridge.state_p3 is not None:
            ...
            print("evader speed_____________££££££££",Xe[0])
        ...
        if captured:
            print(f"Captured _____________by pursuer {i_cap}, distance = {d_min:.2f} m")
...
if __name__ == "__main__":
    ...
    (states_pursuer1, states_pursuer2, states_pursuer3, states_evader,
     ...
     all_pred_path, evader_diag_history) = simulation_results
