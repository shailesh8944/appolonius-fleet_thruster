...
def plot_evader_strategy_diagnostics(diag_history: List[Dict[str, Any]], plots_dir: str, time_step: float = 0.1,
    ...
    for diag in diag_history:
        pair_map = {tuple(tp.get('pair', ())) : tp.get('theta', np.nan) for tp in diag.get('theta_pairs', []) if 'pair' in tp}
        for pair in theta_traces:
            theta_traces[pair].append(pair_map.get(pair, np.nan))

        active_pairs.append(tuple(diag.get('active_pair', (np.nan, np.nan))))
        theta_active.append(diag.get('theta_active', np.nan))
        psi_1.append(diag.get('psi_candidate', np.nan))
        psi_2.append(diag.get('psi_candidate_2', np.nan))
        psi_sel.append(diag.get('psi_selected', np.nan))
        dtheta_1.append(diag.get('dtheta_dt_candidate', np.nan))
        dtheta_2.append(diag.get('dtheta_dt_candidate_2', np.nan))
        phi_i.append(diag.get('phi_i', np.nan))
        phi_j.append(diag.get('phi_j', np.nan))
        r_i.append(diag.get('r_i', np.nan))
        r_j.append(diag.get('r_j', np.nan))
...
def generate_analysis_plots(states_pursuer1, states_pursuer2, states_pursuer3, 
                          ...
                          port_act_p1=None, stbd_act_p1=None, time_step=0.1):
    ...
    # Optional: plot port/stbd actuator commands for Pursuer 1
    def plot_p1_thrusters(port_cmds, stbd_cmds, ts):
        n = min(len(port_cmds), len(stbd_cmds))
        if n == 0:
            return
        t = np.arange(n) * ts
        plt.figure(figsize=(10, 4))
        plt.plot(t, port_cmds[:n], label='Port actuator')
        plt.plot(t, stbd_cmds[:n], label='Starboard actuator')
        plt.xlabel('Time (s)')
        plt.ylabel('Actuator command')
        plt.title('Pursuer 1 Port/Starboard Actuators')
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(plots_dir, 'pursuer1_thrusters.png'))
        plt.close()

    if port_act_p1 is not None and stbd_act_p1 is not None:
        plot_p1_thrusters(np.array(port_act_p1, dtype=float),
                          np.array(stbd_act_p1, dtype=float),
                          time_step)
